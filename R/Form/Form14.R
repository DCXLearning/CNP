# source("global.R")
# source("R/Server.R")
from14_summary <- reactive({
  validate(need(exists("form7_8_10_14_all"),
                "form7_8_10_14_all not found (build it in global.R)"))
  
  df <- form7_8_10_14_all %>%
    dplyr::mutate(
  ...
  k29 <- df %>%
    ...
    summarise(
      `១.ក ផែនការឆមាស`   = round(
        sum(if_else(month == 6, coalesce(semesterplan, 0), 0), na.rm = TRUE),
        2
      ),
  ...
  final <- fix_tranfer2 %>%
    ...
    dplyr::arrange(province_kh, year,semester) %>%
    dplyr::transmute(
