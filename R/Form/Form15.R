# source("global.R")
# source("R/Server.R")
from15_summary <- reactive({
  validate(need(exists("form7_8_10_11_15_final"),
                "form7_8_10_11_15_final not found (build it in global.R)"))
  
  df <- form7_8_10_11_15_final %>%
    dplyr::mutate(
  ...
  #       sum(if_else(month == 1, coalesce(semesterplan, 0), 0), na.rm = TRUE),
  #       2
  #     ),
  #     `១.ក ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  # 
  # # k30
  ...
  #     ),
  # 
  #     # `២.ក ផែនការឆមាស`   = round(sum(coalesce(semesterplan, 0), na.rm = TRUE), 2),
  #     `២.ក ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  # 
  # # k31
  ...
  #       2
  #     ),
  #     `២.ខ ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  # 
  # # k32
  ...
  #       2
  #     ),
  #     `២.គ ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  # 
  # # k33
  ...
  #       2
  #     ),
  #     `២.ឃ ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  # 
  # 
  ...
  #       2
  #     ),
  #     `២.ង ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
  #     .groups = "drop"
  #   )
  k29 <- df %>%
    filter(
    ...
    summarise(
      ...
      # ១.ក ឆមាសអនុវត្ត (Current Semester Spend)
      `១.ក ឆមាសអនុវត្ត` = round(
        sum(if_else(semester == 2, coalesce(current, 0), 0), na.rm = TRUE),
        2
      ),
  ...
  k30 <- df %>%
    ...
    summarise(
      ...
      # `២.ក ផែនការឆ្នាំ`   = round(sum(coalesce(semesterplan, 0), na.rm = TRUE), 2),
      # `២.ក ឆមាសអនុវត្ត` = round(sum(coalesce(current,      0), na.rm = TRUE), 2),
      .groups = "drop"
    )
  ...
  final <- fix_tranfer2 %>%
    ...
    dplyr::arrange(province_kh, year) %>%
    dplyr::transmute(
