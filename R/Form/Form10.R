# source("global.R")
# source("R/Server.R")
from10_summary <- reactive({
  validate(need(exists("form7_8_10_all"),
                "form7_8_10_all not found (build it in global.R)"))
  ...
  final <- fix_tranfer2 %>%
    ...
    dplyr::arrange(province_kh, district_kh, year,month) %>%
    dplyr::transmute(
